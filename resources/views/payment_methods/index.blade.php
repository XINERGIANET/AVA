                    ...
                    <tbody class="text-center">
                        ...
                        <tr style="border-bottom: 1px solid #e9ecef;">
                            ...
                            <td class="text-center">
                                @if ($method->location)
                                    <span class="badge bg-primary px-3 py-2" style="font-size: 0.8rem;"><i class="bi bi-geo-alt-fill me-1"></i>{{ $method->location->name }}</span>
                                @else
                            ...
                            <td class="pe-4 text-center">
                                @if ($isMaster || ($user->location_id && $method->location_id == $user->location_id))
                                    ...
                                    data-id="{{ $method->id }}" data-name="{{ $method->name }}" data-location-id="{{ $method->location_id ?? '' }}" title="Editar">
        ...
        <div class="modal-content">
            <form id="createPaymentMethodForm" action="{{ route('payment-methods.store') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="create_location_id" class="form-label text-dark fw-bold">Sede Asignada</label>
                        @if ($isMaster)
                            <select class="form-select" id="create_location_id" name="location_id">
                                <option value="">-- Todas las Sedes (Global) --</option>
                                @foreach ($locations as $loc)
                                    <option value="{{ $loc->id }}">{{ $loc->name }}</option>
                                @endforeach
                            </select>
                            <small class="text-muted">Si selecciona una sede, el método de pago solo estará disponible para esa sede.</small>
                        @else
                            @php
        ...
        <div class="modal-content">
            <form id="editPaymentMethodForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit_location_id" class="form-label text-dark fw-bold">Sede Asignada</label>
                        @if ($isMaster)
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#editModal').on('show.bs.modal', function(event) {
            ...
            if ($('#edit_location_id').is('select')) {
                $('#edit_location_id').val(locationId !== undefined ? locationId : '');
            }
        });
