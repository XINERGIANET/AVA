...
class PurchaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make(array_merge($request->all(), ['details' => $details]), [
            ...
            'details.*.measurement_unit' => 'nullable|string|max:255',
            'details.*.tank_id'          => 'nullable|exists:tanks,id',
        ]);
        ...
        try {
            ...
            foreach ($processedDetails as $detail) {
                ...
                $tank = Tank::find($detail['tank_id']);
                if ($tank) {
                    $tank->stored_quantity = ($tank->stored_quantity) + $detail['quantity'];
                    $tank->save();
                    
                    // Registrar merma si hay (usar location_id del tanque)
                    if($detail['waste'] > 0){
                        Waste::create([
                            'amount' => $detail['waste'],
                            'type' => 'compras',
